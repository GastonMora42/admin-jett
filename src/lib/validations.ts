...
export class Validator {
  ...
  private validateField(value: unknown, rules: ValidationRule, fieldName: string): string | null {
    // Required validation
...
export const validateClientData = (data: Record<string, unknown>): ValidationResult => {
...
export const validateProjectData = (data: Record<string, unknown>): ValidationResult => {
  return new Validator(data)
    ...
    .field('cuotas', { 
      ...
      custom: (value: unknown) => {
        ...
        const numValue = typeof value === 'number' ? value : Number(value)
        if (data.formaPago === 'MENSUAL' && (!numValue || isNaN(numValue) || numValue < 1)) {
...
export const validateUserData = (data: Record<string, unknown>, isEdit: boolean = false): ValidationResult => {
  const validator = new Validator(data)
    ...
    .field('rol', { 
      required: true,
      custom: (value: unknown) => {
        if (!['SUPERADMIN', 'ADMIN', 'VENTAS'].includes(value as string)) {
          return 'Rol inválido'
        }
        return null
      }
...
export const validatePaymentData = (data: Record<string, unknown>): ValidationResult => {
  return new Validator(data)
...
export const validatePasswordChange = (data: Record<string, unknown>): ValidationResult => {
  const validator = new Validator(data)
...
export const validateSettingsData = (data: Record<string, unknown>): ValidationResult => {
  return new Validator(data)
